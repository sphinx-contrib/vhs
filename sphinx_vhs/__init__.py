...
def download(url, out, name):
    summary = bold(f'downloading {name}... ')

    def dl_report(bn, bs, size):
        done = (bn * bs) // 1024 ** 2
        size = size // 1024 ** 2
        logger.info(f'{summary}[{done}/{size}MB]\r', nonl=True)

    urllib.request.urlretrieve(url, out, reporthook=dl_report)

    logger.info('\n', nonl=True)

def download_vhs():
    if sys.platform != 'linux' or os.environ.get('READTHEDOCS') != 'True':
    ...
    tmp_dir = pathlib.Path(tempfile.mkdtemp())
    try:
        basename = ffmpeg_url.rsplit('/', maxsplit=1)[1]
        download(ffmpeg_url, tmp_dir / basename, 'ffmpeg')
        logger.info(f'{bold("unpacking ffmpeg...")}')
        shutil.unpack_archive(tmp_dir / basename, tmp_dir)

        for f in tmp_dir.glob('**/ffmpeg'):
            if f.stat().st_mode & stat.S_IEXEC:
                os.replace(f, out_dir / 'ffmpeg')
                break
        else:
    ...
    tmp_dir = pathlib.Path(tempfile.mkdtemp())
    try:
        basename = vhs_url.rsplit('/', maxsplit=1)[1]
        download(vhs_url, tmp_dir / basename, 'vhs')
        logger.info(f'{bold("unpacking vhs...")}')
        shutil.unpack_archive(tmp_dir / basename, tmp_dir)

        for f in tmp_dir.glob('**/vhs'):
            if f.stat().st_mode & stat.S_IEXEC:
                os.replace(f, out_dir / 'vhs')
                break
        else:
            raise RuntimeError('failed to download ffmpeg')
    finally:
    ...
    try:
        f = tmp_dir / 'ttyd'
        download(ttyd_url, f, 'ttyd')
        f.chmod(f.stat().st_mode | stat.S_IEXEC)
        os.replace(f, out_dir / 'ttyd')
    finally:
        shutil.rmtree(tmp_dir)
